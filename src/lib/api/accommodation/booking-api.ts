 ...
 * This function combines destination search and hotel search
 */
export async function searchAccommodations(params: {
  ...
  try {
    // First, search for destinations to get dest_id
    const destinationsResponse = await searchDestinations({ query: params.destination });
    ...
    const destination = destinationsResponse.data[0];
    ...
    if (!hotelsResponse.data || !hotelsResponse.data.hotels) {
      return [];
    }
    
    // Transform the results to match the expected format
    return hotelsResponse.data.hotels.map((hotel: any) => ({
      id: hotel.property.id,
      name: hotel.property.name,
      location: `${destination.city_name}, ${destination.country}`,
      ...
      amenities: [], // Booking API doesn't provide amenities in this response
      image_url: hotel.property.photoUrls?.[0] || '',
      booking_url: `https://www.booking.com/hotel/${hotel.property.countryCode}/${hotel.property.id}.html`,
      description: hotel.accessibilityLabel || hotel.property.name,
    }));
  } catch (error) {
